...
class FakeTransport implements LspTransport {
  ...
  void send(String message) {
    ...
    scheduleMicrotask(() {
      _incoming.add(jsonEncode(<String, dynamic>{
        'jsonrpc': '2.0',
        'id': id,
        'result': cannedResult,
      }));
...
void main() {
  test('sends a framed completion request and parses the canned result',
      () async {
    final transport = FakeTransport(cannedResult: <String, dynamic>{
      'isIncomplete': false,
    ...
    final List<LspCompletionItem> items =
        await client.completion('lib/main.dart', 3, 7);
    ...
    expect(params['position'], <String, dynamic>{
      'line': 3,
      'character': 7,
    });
  ...
  test('surfaces publishDiagnostics notifications on diagnosticsStream',
      () async {

// Nova modification notice (Apache-2.0): forked from termux-play-store/termux-exec
// v1.9 (Apache-2.0; see LICENSE.UPSTREAM) for the Nova IDE (sd.adaa.codeide).
// Nova changes: baked-in base/prefix target the Nova sandbox (via
// -DTERMUX_BASE_DIR/-DTERMUX_PREFIX in CMakeLists.txt); log prefix renamed.
// Upstream notices retained; this file remains under Apache-2.0.
#define _GNU_SOURCE
// #include <dlfcn.h>
// #include <string.h>
#include <dlfcn.h>
...
#define LOG_PREFIX "[nova-exec] "

// Check if `string` starts with `prefix`.
static bool starts_with(const char *string, const char *prefix) { return strncmp(string, prefix, strlen(prefix)) == 0; }

// Nova: the baked TERMUX_BASE_DIR is textual, but realpath() canonicalizes
// symlinks (e.g. /data/user/0 vs /data/data differ per device/ROM), so the
// resolved target path may no longer textually start with the baked string.
// Resolve the base itself once and accept either form; otherwise the wrap
// gate fails closed and nested exec dies with EACCES on target 29+.
static char nova_base_canonical[PATH_MAX];
static volatile int nova_base_canonical_done = 0;

static const char *nova_canonical_base(void) {
  if (!nova_base_canonical_done) {
    if (realpath(TERMUX_BASE_DIR, nova_base_canonical) == NULL) {
      nova_base_canonical[0] = '\0';
    }
    nova_base_canonical_done = 1;
  }
  return nova_base_canonical[0] != '\0' ? nova_base_canonical : NULL;
}
...
static const char *termux_rewrite_executable(const char *executable_path, char *buffer, int buffer_len) {
  ...
  // Nova: compatibility rewrite for mainline Termux-built binaries (used
  // until the Nova package repo exists). Some binaries construct exec paths
  // at RUNTIME with the hardcoded com.termux prefix (e.g. Node's libuv
  // spawning "/data/data/com.termux/files/usr/bin/sh"), which byte-patching
  // can never fix. Map that prefix to our baked base dir here instead.
  // NOTE: keep in sync with the byte-patch needle in BootstrapInstaller
  // ("/data/data/com.termux/files/usr") while the transition lasts.
  static const char legacy_base[] = "/data/data/com.termux/files/";
  if (strncmp(executable_path, legacy_base, sizeof(legacy_base) - 1) == 0) {
    size_t base_len = strlen(TERMUX_BASE_DIR);
    size_t rest_len = strlen(executable_path + sizeof(legacy_base) - 1);
    if (base_len + 1 + rest_len < (size_t)buffer_len) {
      memcpy(buffer, TERMUX_BASE_DIR, base_len);
      buffer[base_len] = '/';
      memcpy(buffer + base_len + 1, executable_path + sizeof(legacy_base) - 1, rest_len + 1);
      return buffer;
    }
    return executable_path;
  }
...
__attribute__((visibility("default"))) int execve(const char *executable_path, char *const argv[], char *const envp[]) {
  ...
  bool wrap_in_linker = (strstr(path_to_use, TERMUX_BASE_DIR) == path_to_use)
                        || (nova_canonical_base() != NULL
                            && strstr(path_to_use, nova_canonical_base()) == path_to_use)
...
// Nova: libuv (Node) and some runtimes spawn via posix_spawn(), which may
// bypass the PLT execve symbol above (observed: no intercept trace for
// node's /bin/sh spawn while fork+execve siblings intercept fine). Route the
// path through the same rewrite policy, then delegate to the real
// posix_spawn via dlsym(RTLD_NEXT). The wrapped child still passes through
// our execve interceptor for the linker decision, so this stays a thin,
// auditable layer. Debug traces mirror the execve interceptor.
typedef int (*nova_real_spawn_fn)(pid_t *, const char *, const posix_spawn_file_actions_t *,
                                  const posix_spawnattr_t *, char *const *, char *const *);

static int nova_spawn_via_real(const char *sym, const char *label, pid_t *pid, const char *path,
                               const posix_spawn_file_actions_t *file_actions,
                               const posix_spawnattr_t *attrp, char *const argv[], char *const envp[]) {
  static nova_real_spawn_fn cached_spawn = NULL;
  static nova_real_spawn_fn cached_spawnp = NULL;
  bool is_p = (sym[11] == 'p');  // "posix_spawn" vs "posix_spawnp".
  nova_real_spawn_fn *slot = is_p ? &cached_spawnp : &cached_spawn;
  if (*slot == NULL) {
    *slot = (nova_real_spawn_fn)dlsym(RTLD_NEXT, sym);
    if (*slot == NULL) {
      return ENOSYS;
    }
  }
  bool debug = getenv("TERMUX_EXEC_DEBUG") != NULL;
  if (getenv("TERMUX_EXEC_OPTOUT") != NULL || path == NULL) {
    return (*slot)(pid, path, file_actions, attrp, argv, envp);
  }
  if (debug) {
    fprintf(stderr, LOG_PREFIX "Intercepting %s('%s'):\n", label, path);
  }
  char buffer[PATH_MAX];
  const char *use_path = termux_rewrite_executable(path, buffer, sizeof(buffer));
  if (debug && use_path != path) {
    fprintf(stderr, LOG_PREFIX "Rewritten path: '%s'\n", use_path);
  }
  return (*slot)(pid, use_path, file_actions, attrp, argv, envp);
}
...
__attribute__((visibility("default"))) int posix_spawn(pid_t *pid, const char *path,
  ...
  return nova_spawn_via_real("posix_spawn", "posix_spawn", pid, path, file_actions, attrp, argv, envp);
}

__attribute__((visibility("default"))) int posix_spawnp(pid_t *pid, const char *file,
    const posix_spawn_file_actions_t *file_actions, const posix_spawnattr_t *attrp,
    char *const argv[], char *const envp[]) {
  return nova_spawn_via_real("posix_spawnp", "posix_spawnp", pid, file, file_actions, attrp, argv, envp);
}

...
class BootstrapInstaller(private val context: Context) {
    ...
    private fun patchHardcodedPaths(prefix: File) {
        ...
        // Our package is sd.adaa.nova (prefix = /data/user/0/sd.adaa.nova/files/usr).
        ...
        //   /data/data/com.termux/files/usr (31) -> /data/user/0/sd.adaa.nova/abcde (31)
        // We create a symlink at appDir/abcde -> files/usr, so the patched path resolves correctly.
        val filesDir = prefix.parentFile ?: return
        val appDir = filesDir.parentFile ?: filesDir
        // abcde must be at /data/user/0/sd.adaa.nova/abcde -> files/usr
        val abcdeLink = File(appDir, "abcde")
        try {
            if (!abcdeLink.exists()) {
                Os.symlink("files/usr", abcdeLink.absolutePath)
                android.util.Log.i("BootstrapInstaller", "Created abcde symlink for prefix patch at ${abcdeLink.absolutePath}")
            }
        } catch (_: Exception) {}
        ...
        val replacementBase = "/data/user/0/sd.adaa.nova/abcde".toByteArray(Charsets.UTF_8)
        ...
        fun patchFile2(file: File) {
            ...
            try {
                ...
                while (i <= bytes.size - needle2.size) {
                    ...
                    if (match) {
                        // Replace with /data/user/0/sd.adaa.nova padded to 22 with nulls
                        // /data/user/0/sd.adaa.nova is 26, too long, so we use /data/data/sd.adaa (19) padded, but we already have x trick for the longer one.
